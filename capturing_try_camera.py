...
import tetris_pixel.methods as methods
import tetris_pixel.Contours as Contours
import tetris_pixel.config as config
import tetris_pixel.init as init
from tetris_pixel.encode import pieces
    ...
    for idx in range(len(config.hsv)):
        ...
        if len(contours) > 0 and len(contours) < 6: #f"Warning! No contour detected at idx = {idx}"
            for contour in contours:
                methods.draw(src, contour, obv = 4)
            after_find_cnt = time.time()
            shape_prop = Contours.contour_property(src, idx, contours)
            if shape_prop:
